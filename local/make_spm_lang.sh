...
### NOTE: for Librispeech we take away ' here - it is handled separately.
#sed -e '/^<unk>$/d' -e '/^<s>$/d' -e '/^<\/s>$/d' -e '/^▁$/d' -e "/^'$/d" <${vocab} > ${tmpdir}/vocab.filtered
...
# Add some extra words to placeholder lexicon. The sentencepiece space is handled by subword-kaldi/local/make_lfst_spm.py
echo -e ".fp\tSPN" >> ${dictdir}/lexicon_placeholders.txt
echo -e ".ct\tNSN" >>  ${dictdir}/lexicon_placeholders.txt
echo -e ".cough\tNSN" >> ${dictdir}/lexicon_placeholders.txt
echo -e ".laugh\tNSN" >> ${dictdir}/lexicon_placeholders.txt
echo -e ".yawn\tNSN" >> ${dictdir}/lexicon_placeholders.txt
echo -e ".sigh\tNSN" >> ${dictdir}/lexicon_placeholders.txt
echo -e ".br\tNSN" >> ${dictdir}/lexicon_placeholders.txt
