...
from mha import MultiHeadLocationAwareAttention
...
class AttentionalRNNDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.attn_internal_dim = attn_internal_dim
        self.num_layers = num_layers
        ...
        elif self.attn_type == "mhalocation":
            self.attn = MultiHeadLocationAwareAttention(
                enc_dim=enc_dim,
                dec_dim=self.hidden_size,
                attn_dim=self.attn_dim,
                output_dim=self.attn_dim,
        ...
        elif self.attn_type == "mhalocationwide":
            self.attn = MultiHeadLocationAwareAttention(
                enc_dim=enc_dim,
                dec_dim=self.hidden_size,
                attn_dim=self.attn_internal_dim,
                output_dim=self.attn_dim,
                conv_channels=self.channels,
                kernel_size=self.kernel_size,
                scaling=self.scaling,
